...
class Command(BaseCommand):
    ...
    def _scaffold_component(  # noqa: PLR0913 - one explicit generation request
        ...
        *,
        ...
        if needs_js and names.class_name == "WeakMap":
            message = "Choose a JavaScript name that does not shadow the scaffold's WeakMap dependency."
            raise CommandError(message)
    ...
    def _module_bindings(source: str, path: Path) -> set[str]:
        """Include imports, assignments and definitions, not only function names."""
        table = symtable.symtable(source, str(path), "exec")
        return {symbol.get_name() for symbol in table.get_symbols() if symbol.is_assigned() or symbol.is_imported()}
    ...
    def _validate_new_component(self, ui_path: Path, names: ComponentNames, category: str) -> None:
        ...
        if any(self._snapshot(path) is not None for path in paths):
            message = f"Component {names.slug!r} already exists; no files written."
            raise CommandError(message)
        tags = ui_path / "templatetags/insight_tags.py"
        try:
            source = self._read_file(tags)
            configs = self._read_file(ui_path / "configs/__init__.py")
        except OSError as exc:
            message = f"Incomplete source checkout: {exc}"
            raise CommandError(message) from exc
        if names.slug in self._module_bindings(source, tags) or re.search(
...
"""
    ...
    def _create_config_dataclass(  # noqa: PLR0913, PLR0917
        ...
        if config_class_name in self._module_bindings(content, file_path):
