...
def test_navbar_and_footer_defaults_use_central_brand_settings() -> None:
    """Navbar and footer defaults should read the same public brand source."""
    navbar_config = get_navbar_brand_defaults()
    footer_description = get_footer_description_defaults()

    assert navbar_config.request_url == "/portal/"
    assert navbar_config.mark.primary_text == "Acme Portal"
    assert navbar_config.mark.logo.alt == "Acme Logo"
...
def test_brand_logo_can_use_login_specific_height() -> None:
    """A host login screen can share the brand logo with a larger layout height."""
