...
class TestSearchBar(TemplateTagsTestCase):
    ...
    def test_search_bar_renders_configured_search_index_url(self) -> None:
        """Host applications can supply their own localized search index."""
        rendered = self.render_template(
            "{% load insight_tags %}"
            '{% search_bar enable_search=True search_index_url="/static/host-search/search-index-de.json" %}'
        )
        search_container = BeautifulSoup(rendered, "html.parser").select_one("[data-insight-search]")

        assert search_container is not None
        assert search_container["data-search-index"] == "/static/host-search/search-index-de.json"
